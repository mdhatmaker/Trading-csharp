...
namespace TT.SP.Trading.Controls.MDTrader
{
    public partial class SPMDTraderControl : UserControl
    {
        ...
        //recenter on our first update, whether it is from cached data, or when we get our first update from instrument
        ...
        private void RefreshWorkingAtPriceLevel(int priceLevel)
        {
            ...
            {
                //TODO: tpeterson - I think this should be different, depending on whether
                //the stop order is triggered (then should display at Limit price) or whether the
                //stop price is set to a value that would make the Limit part of the order either 
                //always inactive or always active. There is also the question of whether the exchange
                //would allow the stop portion set to a very large or very small number like int.MaxValue or int.MinValue

                // display stop orders at their stop price
                if (order.IsStop && IsValidStop(order.StopPrice) )
                {
            ...
            if (_middleTag == 0)
            {
                if (_imuPublisher != null)
                {
                    ...
                    if (cachedIMU != null)
                    {
                        ...
                        //tpeterson - changed to use TT_INVALID_PRICE 5/28/08
                        if (cachedIMU.Bid != TT_CONSTANTS.TT_INVALID_PRICE && cachedIMU.Ask != TT_CONSTANTS.TT_INVALID_PRICE)
                            _middleTag = cachedIMU.Bid + ((cachedIMU.Ask - cachedIMU.Bid) / 2);
                        else if (cachedIMU.Bid != TT_CONSTANTS.TT_INVALID_PRICE)
                            _middleTag = cachedIMU.Bid;
                        else if (cachedIMU.Ask != TT_CONSTANTS.TT_INVALID_PRICE)
                            _middleTag = cachedIMU.Ask;
                        else if (cachedIMU.Last != TT_CONSTANTS.TT_INVALID_PRICE)
        ...
        {
            int levelToRefresh = 0;
            if (order.IsStop && (order.IsTriggered == false) )
            ...
            if (order.PreviousChangeLimitPrice != TT_CONSTANTS.TT_INVALID_PRICE)
        ...
        //tpeterson - probably should work to get only TT_INVALID_PRICE used for Stop prices, but 
        //right now there appear to be cases that depend on either int.MaxValue or int.MinValue
        private bool IsValidStop(int stopPrice)
        {
            return ((stopPrice != TT_CONSTANTS.TT_INVALID_PRICE) &&
                        (stopPrice != int.MinValue) &&
                        (stopPrice != int.MaxValue));
        }
        ...
        private void MDTrader_ChangeOrdersAtLevel(int origLevel, int newLevel)
        {
            ...
            {
                if ( IsValidStop(ord.StopPrice) && ord.StopPrice == origLevel)
        ...
        public void Clear()
        {
            ...
            mdTraderControlMain.SetLTP(TT_CONSTANTS.TT_INVALID_PRICE);
            ...
            IExchangeStatePublisher exchangeStatePublisher)
        {
            ...
            if (imuPublisher != null)
            {
                ...
                if (depthPublisher == null)
                {
                    ...
                    if (_imuPublisher.Started)
                    {
                        IInsideMarketUpdate latestUpdate = _imuPublisher.GetLatestInsideMarketUpdate();
                        if (latestUpdate != null)
                        {
                            IMPublisher_InsideMarketUpdatedRecenter(_imuPublisher, latestUpdate);
                        }
                    }
            ...
            if (depthPublisher != null)
            {
                ...
                if (_depthPublisher.Started)
                {
                    IDepthUpdate latestUpdate = _depthPublisher.GetLatestDepthUpdate();
                    if (latestUpdate != null)
                    {
                        DepthPublisher_DepthUpdateRecenter(depthPublisher, latestUpdate);
                    }
                }
